...
class QUNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    @staticmethod
    def from_unet(unet: UNet2DConditionModel, 
                  weight_quant: FloatingPoint = None,
                  weight_flex_bias: bool = False,
                  qargs: Optional[Dict[str, Any]] = None,
                  repeat_module_count: int = 1,
                  repeat_model_count: int = 1,
        ...
        config = unet.config
        config["_class_name"] = "QUNet2DConditionModel"

        qnet = QUNet2DConditionModel.from_config(config)

        qnet.load_state_dict(unet.state_dict())
            ...
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                ...
                for exl in exclude:
                    if exl in name:
                        ...
                        break
                if flag:
                    quant = make_block_quantizer(**qargs)
                    ##new_module = torch.nn.Sequential(quant, module)
                    new_module = Qop(quant, module)
                    ...
                    setattr(sub_module, name_parts[-1], new_module)
    ...
    def move_computation_modules_to_repeat_module_count(self, repeat_module_count: int, 
                                                        layer_stats: bool = False):
        r"""
        Moves the computation modules to the repeat module.
        """


        if repeat_module_count == 1 and not self.dynamic_repeats:
            return
        
        include = [Downsample2D, FirDownsample2D, FirUpsample2D, KDownsample2D, KUpsample2D, ResnetBlock2D, Upsample2D]
        include += [Attention, FeedForward]
            ...
            for class_to_include in include:
                if isinstance(module, class_to_include):
                    ...
                    if not layer_stats:
                        ...
                        repeat_module = RepeatModuleStats(module, repeat_module_count, name, False, repeat_counter)
                        repeat_counter += 1
    ...
    def check_for_nested_repeat_module_count(self, repeat_module_count: int, layer_stats: bool = False):
        r"""
        Checks for nested repeat modules.
        """
        ...
        for name1, module in self.named_modules():
            if isinstance(module, RepeatModule):
                repeat_counter += 1
                for name2, sub_module in module.named_modules():
                    if name2 == "":
                        continue
                    if isinstance(sub_module, RepeatModule):
                        print(f"Nested repeat module found in  {name1} and {name1}.{name2}")
                        nested_repeats = True
    ...
    def set_number_of_repetitions(self, repeat_module_count: int):
        r"""
        Sets the number of repetitions for the repeat module.
        """
    ...
    def get_repetition_at_timestep(self, timesteps: torch.tensor):
        r"""
        Gets the repetition at a given timestep.
        """
        
        if self.timestep_to_repetition is None:
            raise ValueError("The repetition plan has not been set.")
        ...
        return self.timestep_to_repetition[timestep]
    ...
    def forward(
        ...
        if self.dynamic_repeats:
            num_repetitions = self.get_repetition_at_timestep(timestep)
            if self.last_repetition_count != num_repetitions:
                self.set_number_of_repetitions(num_repetitions)
                print(f"Repetition count set to {num_repetitions}")
                self.last_repetition_count = num_repetitions
