...
class QuantizedEulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    """
    Euler scheduler.
    ...
    def __init__(
        ...
        self.quantized_run = quantized_run
        ...
        ##self.prepare_for_noise(quantization_noise, gamma_threshold, num_train_timesteps, sigmas)
    ...
    def make_repetition_plan(self):
        r"""
        Adjusts the repetition plan.
        """

        repetitions = self.repetitions
        timesteps = self.timesteps

        assert len(repetitions) == len(timesteps), "Repetitions and timesteps should have the same length."

        timestep_to_repetition = {timesteps[i].item(): repetitions[i].cpu().item() for i in range(len(timesteps))}


        total = self.repetitions.sum().item()
        if wandb.run is not None:
            wandb.log("total number of repetitions: ", total, commit=False)

        return timestep_to_repetition

    def prepare_for_noise(self, 
                          quantization_noise: Optional[Union[torch.Tensor, float]] = None, 
                          gamma_threshold: float = 0.0,
                          num_train_timesteps: int = 1000,
        ...
        if quantization_noise is not None:
            if isinstance(quantization_noise, float):
                quantization_noise = torch.tensor([quantization_noise])

            if quantization_noise.shape[0] == 1:
                quantization_noise = quantization_noise.repeat(num_train_timesteps)
            elif quantization_noise.shape[0] != num_train_timesteps:
                raise ValueError("quantization_noise must have length 1 or num_train_timesteps")
            

            if not isinstance(sigmas, torch.Tensor):
                tsigmas = torch.tensor(sigmas, dtype=torch.float32)
            else:
                tsigmas = sigmas
            ...
            elif self.quantization_noise_mode == 12:
                quantization_noise[:] = quantization_noise / 2
            elif self.quantization_noise_mode == 13:
                quantization_noise[:] = quantization_noise / 4
            elif self.quantization_noise_mode == 14:
                quantization_noise[:] = quantization_noise / 5
            elif self.quantization_noise_mode == 15:
                quantization_noise[:] = quantization_noise / 8
    ...
    def step(
        ...
        if wandb.run is not None:
            ...
            report_dict = {"gamma": gamma,
                           "timestep": timestep,
                           "sigma": sigma,
                           "sigma_hat": sigma_hat,
                           "dt": dt,
                           "step_index": self._step_index,
                           "repeats": self.repetitions[self.step_index],
                           }
